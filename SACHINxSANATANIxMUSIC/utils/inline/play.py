...
def stream_markup(_, chat_id):
    buttons = [
        [
            ...
            InlineKeyboardButton(text="II", callback_data=f"ADMIN Pause|{chat_id}"),
            InlineKeyboardButton(text=">", callback_data=f"ADMIN Skip|{chat_id}"),
            InlineKeyboardButton(text="↻", callback_data=f"ADMIN Replay|{chat_id}")↻▢II▷
        ],
        [
            InlineKeyboardButton(
                text="🎧 𝐎𝐖𝐍𝐄𝐑 🎧", url=f"tg://openmessage?user_id={OWNER_ID}",
...
def playlist_markup(_, videoid, user_id, ptype, channel, fplay):
    buttons = [
        [
            InlineKeyboardButton(
                text=_["P_B_1"],
                callback_data=f"AyanoPlaylists {videoid}|{user_id}|{ptype}|a|{channel}|{fplay}",
            ),
            InlineKeyboardButton(
                text=_["P_B_2"],
                callback_data=f"AyanoPlaylists {videoid}|{user_id}|{ptype}|v|{channel}|{fplay}",
            ),
        ],
        [
            InlineKeyboardButton(
